    ...
    with col2:
        ...
        if uploaded_file is None:
            ...
            if st.button("▶ INITIATE AI ANALYSIS", type="primary", use_container_width=True):
                ...
                # Save History
                scan_record = {"Date": datetime.datetime.now().strftime("%H:%M:%S"), "Patient": patient_name, "Diagnosis": result_text}
                st.session_state.scan_history.append(scan_record)

                st.download_button("📄 DOWNLOAD REPORT", data=f"Report for {patient_name}: {result_text}", file_name="Report.txt", use_container_width=True)
